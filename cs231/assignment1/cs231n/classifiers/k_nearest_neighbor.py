...
class KNearestNeighbor(object):
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in xrange(num_test):
      ...
      #######################################################################
      dists[i] = np.sum(np.square(X[i] - self.X_train),axis = 1)   # 无需考虑维度问题 会自动进行填充
      pass
      #######################################################################
      #                         END OF YOUR CODE                            #
      #######################################################################
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      closest_y = self.y_train[np.argsort(dists[i])[:k]]  # 排序后在新的位置上的 原来元素的坐标
      pass
      #########################################################################
      # TODO:                                                                 #
      ...
      for i in range(k):
        if closest_y[i] not in _dic.keys():  # key:label  value:出现次数
          ...
          _dic[closest_y[i]] = _dic[closest_y[i]] + 1 
      _max = max(_dic.values())
